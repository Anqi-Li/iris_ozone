...
#maybe restructure the if statements here
...
def f(i, n):
    alt_chop_cond = (altitude[n].isel(time=i) > bot) & (
        altitude[n].isel(time=i) < top
    )  # select altitude range
    if alt_chop_cond.sum() < 2:
        print(
            "wrong alt range {}/{} in orbit {}".format(
                i - image_lst[n][0], len(image_lst[n]), orbit_nr[n]
            )
        )
        pass
    elif l1[n].isel(time=i).notnull().sum() == 0:
        print(
        ...
        pass
    elif error[n].isel(time=i).notnull().sum() == 0:
        print(
        ...
        try:
            ...
            background_atm = clima.sel(
                lat=ir[n].latitude.isel(time=i),
                month=Time(ir[n].time[i], format="datetime64").datetime.month,
                method="nearest",
            )
            # make apriori ver
            o3_clima = background_atm.o3.sel(lst=lst[n].isel(time=i), method="nearest")
            T_a = background_atm.T
            m_a = background_atm.m
            p_a = background_atm.p
            sol_zen = ir[n].sza[i].values.item()
            forward_args = (
                T_a.values,
                m_a.values,
            ...
            h = (
                altitude[n].isel(time=i)
                .where(l1[n].isel(time=i).notnull(), drop=True)
            ...
            limb = normalise * l1[n].isel(time=i).sel(pixel=h.pixel).values
            error_2 = (normalise * error[n].isel(time=i).sel(pixel=h.pixel).values) ** 2
            ...
            hour_after_sunrise = cal_sunrise_h(
                ir[n].latitude.isel(time=i), ir[n].longitude.isel(time=i), ir[n].time[i]
            )
            ...
            ).reindex(pixel=l1[n].pixel)

            return (
                day_time_lst[n][i].values,
                # ver retrieval
                ver_hat,
                ver_mr,
                ver_mr_rel,
...
#%%
def run_and_save(image_lst, orbit_nr, n):
    result = []
    for i in image_lst:
        result.append(f(i, n))  # run and append retrieval
        ...
        ds = ds.update(
            {
                ...
                "longitude": (
                    ["time"],
                    ir[n].longitude.sel(time=time),
                    {"units": "deg E"},
                ),
                "latitude": (["time"], ir[n].latitude.sel(time=time), {"units": "deg N"}),
                "sza": (["time"], ir[n].sza.sel(time=time), {"units": "deg"}),
                "lst": (["time"], lst[n].sel(time=time), {"units": "hour"}),
                "orbit": (
                    ["time"],
                    np.ones(len(time), dtype="int32") * ir[n].orbit.item(),
                    {"name": "orbit number"},
                ),
            }
        )
...
#client.connect('oso-1.oso.chalmers.se', username = '', password = '')
#path = ['/data2/mof/osiris/StrayLightCorrected/Channel3/']
#sftp = client.open_sftp()

import re
args = {}
args['retrieve_ver_o3_loop_v5.py'] = sys.argv[0]
...
osiris_start = np.int(args['osiris_start'])
osiris_end = np.int(args['osiris_end'])

osiris_files = [file for file in listdir('./data/unprocessed/') if isfile(join('./data/unprocessed/', file))][osiris_start:osiris_end]

ir = [0] * len(osiris_files)
day_time_lst = [0] * len(osiris_files)
file_errors = []

for osiris_file in osiris_files:
    n = osiris_files.index(osiris_file)
    filename = osiris_file
    path = './data/unprocessed/'
    ir[n] = xr.open_dataset(path + filename)  # , chunks={"time": 100})
    ir[n] = ir[n].sel(time=~ir[n].indexes["time"].duplicated())
    day_time_lst[n] = ir[n].time[ir[n].sza < 90]  # select only dayglow
    ir[n] = ir[n].sel(time=day_time_lst[n])
    if len(ir[n].time.searchsorted(ir[n].time)) == 0:
        file_errors.append(filename)

osiris_files = [x for x in osiris_files if x not in file_errors]
ir = [0] * len(osiris_files)
day_time_lst = [0] * len(osiris_files)
altitude = [0] * len(osiris_files)
l1 = [0] * len(osiris_files)
error = [0] * len(osiris_files)
lst = [0] * len(osiris_files)
orbit_nr = [0] * len(osiris_files)
image_lst = [0] * len(osiris_files)
...
for osiris_file in osiris_files:
    ...
    altitude[n] = ir[n].altitude.sel(pixel=sel_pixels)
    l1[n] = ir[n].data.sel(pixel=sel_pixels)
    error[n] = ir[n].sel(pixel=sel_pixels).error
    lst[n] = ir[n].apparent_solar_time
    orbit_nr[n] = np.unique(ir[n].orbit)[0]  # find orbit in dataset
    image_lst[n] = ir[n].time.searchsorted(ir[n].where(ir[n].orbit == orbit_nr[n], drop=True).time)  # get list of all images

def osiris_processor(osiris_file):
    run_and_save(image_lst[osiris_files.index(osiris_file)][0:2], orbit_nr[osiris_files.index(osiris_file)], osiris_files.index(osiris_file))
    gc.collect()

#%%%%%%%%%%%%%%%%%%%% main part #####################
if __name__ == "__main__":    
    for n in range(len(osiris_files)):
        osiris_processor(osiris_files[n])

print ("Program takes", time.time() - start_time, " seconds to run.")
print ('Orbits which are faulty:', file_errors)
#sftp.close()
#client.close()
